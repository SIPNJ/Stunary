...
function drawPiece(clockctx, radius, txtclr){
    let ang;
    clockctx.fillStyle = txtclr;
    clockctx.font = radius * 0.15 + "px arial";
    clockctx.textBaseline = "middle";
    ...
    for(let i = 0; i < 60; i++){
        ang = i * Math.PI / 30;
        clockctx.rotate(ang);
        clockctx.translate(0, -radius * 0.5);
        clockctx.rotate(-ang);
        clockctx.fillText("-", 0, 0);
        clockctx.rotate(ang);
        clockctx.translate(0, radius * 0.5);
        clockctx.rotate(-ang);
    }
...
function drawClock(){
    ...
    drawPieSlice(clockctx, radius, 0, 2 * Math.PI, `blanchedalmond`); // Clock
    drawPieSlice(clockctx, radius-20, 0, 2 * Math.PI, `#766A50`); // Clock inside
    drawPieSlice(clockctx, radius-40, 0, 2 * Math.PI, `blanchedalmond`); // Clock inside inner
    drawPieSlice(clockctx, 5, 0, 2 * Math.PI, `black`); // Point
    drawNumbers(clockctx, radius, "black"); // Number
    drawPiece(clockctx, radius, "black"); //Piece
